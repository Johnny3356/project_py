...
def update_full_slacks(cellgraph: Dict[str, CellNode],
    ...
    for inst in block.getInsts():
        ...
        input_terms = [it for it in inst.getITerms() if it.isInputSignal() and it.getNet().getSigType()=="SIGNAL"]
        output_terms = [it for it in inst.getITerms() if it.isOutputSignal()]
        slacks = []
        endpoints = []
        total_n_slack = 0.0
...
# # --------------------------貪婪greeeeeeeeeedy----------------------------------
N = 150
# neg_nodes = [node for node in cellgraph.values() if node.features['slack'] < 0.0]# 篩選出所有 slack < 0 的節點，並按 slack 越負越前排序
...
for epoch in range(8):
    ...
    # 2) 重新找出负 slack 并排序
    slew_nodes = [n for n in cellgraph.values()if n.features['slack'] < 0.0]
    slew_nodes.sort(key=lambda n: n.features['slack'])
    for i, node in enumerate(slew_nodes[:N]):
        inst_name = node.name
        inst = block.findInst(inst_name)
        if not inst:
            continue
...
# # ---------------------------------final report-------------------------------------
# # ---------------------------------csv file列印--------------------------------------
# csv_file = Path(f"sa_runs_{design_name}.csv")
...
#                     f"{design_name}",        # 或自己填 design 名DESIGN_PATH.name
#                     SEED,
